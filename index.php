<?php include "header-link.php"; ?>
<?php include "header.php"; ?>
<section id="featured">
  <div class="container custom-w">
    <div class="">
      <div class=""> 
        <div class="theme-default">
          <div id="slider" class="nivoSlider">
			      <img class="nivo-main-image" src="files/slider1.jpg" style="display: inline; overflow: hidden; height: 564.122px;">
            <div class="nivo-caption"></div>
            <div class="nivo-directionNav">
              <a class="nivo-prevNav">Prev</a>
              <a class="nivo-nextNav">Next</a>
            </div>
            <div class="nivo-box" name="0" rel="0" style="opacity: 0.1403; left: 0px; top: 0px; width: 169px; height: 678px; overflow: hidden;">
              <img src="files/slider2.jpg" style="position:absolute; width:1349px; height:auto; display:block; top:-0px; left:-0px;">
            </div>
            <div class="nivo-box" name="1" rel="0" style="opacity: 0; left: 169px; top: 0px; width: 169px; height: 678px;">
              <img src="files/slider3.jpg" style="position:absolute; width:1349px; height:auto; display:block; top:-0px; left:-169px;">
            </div>
            <div class="nivo-controlNav">
              <a class="nivo-control" rel="0">1</a><a class="nivo-control" rel="1">2</a>
              <a class="nivo-control" rel="2">3</a><a class="nivo-control active" rel="3">4</a>
              <a class="nivo-control" rel="4">5</a>
            </div>
          </div> 
        </div>
      </div>
      <marquee>
        <p class="MsoNormal">An Autonomous Organization Regd. Under the Indian Public Act., N.C.T. New Delhi Govt. of India. An ISO 9001:2015 Certified Organization Affiliated member of Planning Commission Govt. of India Course Registered under C.R., Ministry of Human Resource Development&nbsp; Govt. of INDIA. N.A.R.C.I Govt. of India .
        </p>
      </marquee>
    </div>
</section>
<section class="welcome">
  <div class="container">
	  <div class="col-lg-12">
      <div class="big-cta">
        <div class="cta-text">
          <h1>Jatiya Yuva Board of Computer Education</h1><br>
        </div>
      </div>
    </div>
  </div>
</section>
<section class="callaction">
  	<div class="form-edu">
        <div class="container">
        <div class="col-md-12">
        	<div class="col-md-8">
                <div class="row home-veri">
                	<div class="col-md-6 marginbot30">
                    <form action="http://www.jybce.org/idVerification" method="post" id="contactform">
                      <input name="action" value="submit" type="hidden">
                      <div class="result">
                        <label>Enter Your ID No</label>
                        <input class="wp-form-control wpcf7-text" name="id_no" id="id_no" required="" type="text" placeholder="Student ID No*">
                        <p class="click"></p>
                        <button class="btn btn-theme margintop10 pull-left" style="float:left" type="submit">Submit</button>
                      </div>
                    </form>
                    </div>
                    <div class="col-md-6 marginbot30">
                    <form action="http://www.jybce.org/resultVerification" method="post" id="contactform">
                      <input name="action" value="submit" type="hidden">
                      <div class="result">
                        <label>Enter Your Enrollment No:</label>
                        <input class="wp-form-control wpcf7-text" name="result_no" id="result_no" required="" type="text" placeholder="Student Enrollment No*">
                        <p class="click"></p>
                        <button class="btn btn-theme margintop10 pull-left" style="float:left" type="submit">Submit</button>
                      </div>
                    </form>
                    
                    </div>
                    <div class="col-md-6">
                    <form id="contactform">
                      <div class="result">
                        <label>Student Login</label>
                        <input name="submit" value="submit" type="hidden">
                        <input class="wp-form-control wpcf7-text" required="" type="text" placeholder="User ID*">
                        <input class="wp-form-control wpcf7-text" required="" type="password" placeholder="Password*">
                        <button class="btn btn-theme margintop10 pull-left" style="float:left" type="submit">Login</button>
                      </div>
                    </form>
                    </div>
                    <div class="col-md-6">
                    <form action="http://www.jybce.org/login" method="post" id="contactform">
                      <div class="result">
                        <label>Centre Login</label>
                        <input name="submit" value="submit" type="hidden">
                        <input class="wp-form-control wpcf7-text" name="user" id="user" required="" type="text" placeholder="User ID*">
                        <input class="wp-form-control wpcf7-text" name="pass" id="pass" required="" type="password" placeholder="Password*">
                        <button class="btn btn-theme margintop10 pull-left" style="float:left" type="submit">Login</button>
                      </div>
                    </form>
                    </div>        
        		</div>
            </div>
            <div class="col-lg-4">
        <aside class="left-sidebar">
<div class="widget">
<div class="panel panel-default">
          <div class="panel-heading"> <i class="fa fa-newspaper-o"></i> <b>News &amp; Events</b></div>
          <div class="panel-body">
            <div class="row">
              <div class="col-xs-12">
                <ul class="demo2" style="overflow-y: hidden; height: 69px;"><li style="overflow: hidden;" class="news-item"> <h5>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.</h5></li>
                                    
                                  </ul>
              </div>
            </div>
          </div>
          <div class="panel-footer"> </div>
        </div>
</div>
          
        </aside>
      </div>
        </div>
    	<div class="clear"></div>
    </div>
  </div>
  
</section>



<section id="content">
<!-- <section class="student-sec">
  <div class="container">
    <div class="row">
      <div class="col-lg-12">
      <ul class="ch-grid">
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>ID Verification</h3>
                      <p><a href="http://www.jybce.org/idVerification">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-1"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Certification Verification</h3>
                      <p><a href="http://www.jybce.org/certificateVerification">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-2"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Result Verification</h3>
                      <p><a href="http://www.jybce.org/resultVerification">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-3"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Online Test</h3>
                      <p><a href="http://www.jybce.org/homepage">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-4"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Download</h3>
                      <p><a href="http://www.jybce.org/download">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-5"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Our Courses</h3>
                      <p><a href="http://www.jybce.org/course">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-6"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Gallery</h3>
                      <p><a href="http://www.jybce.org/gallery">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-7"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Training Centre</h3>
                      <p><a href="http://www.jybce.org/study-centre">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-8"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Publication</h3>
                      <p><a href="http://www.jybce.org/publication">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-9"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Bank Details</h3>
                      <p><a href="http://www.jybce.org/bank-details">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-10"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Centre Login</h3>
                      <p><a href="http://www.jybce.org/centerLogin">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-11"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Student Login</h3>
                      <p><a href="http://www.jybce.org/library">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-12"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Exam Section</h3>
                      <p><a href="http://www.jybce.org/homepage#">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-13"></div>
              </div>
          </li>
          <li>
              <div class="ch-item">
                  <div class="ch-info">
                      <h3>Suggestions</h3>
                      <p><a href="http://www.jybce.org/suggestions">Click Here</a></p>
                  </div>
<div class="ch-thumb ch-img-14"></div>
              </div>
          </li>
      </ul>
      
      
      </div>
      
    </div>
  </div>
</section>  -->
  <section class="enquiry-sec">
  <div class="container">
    <div class="row">
      <div class="col-lg-12">
<div class="content">
            <div id="carousel-example-generic" class="carousel slide" data-ride="carousel"> <!-- Indicators -->
              <!-- Wrapper for slides -->
              <div class="carousel-inner">
				                
              </div>
              <!-- Controls --> 
              <a class="left carousel-control" href="http://www.jybce.org/homepage#carousel-example-generic" data-slide="prev"><i class="fa fa-chevron-left" aria-hidden="true"></i></a>
              <a class="right carousel-control" href="http://www.jybce.org/homepage#carousel-example-generic" data-slide="next"><i class="fa fa-chevron-right" aria-hidden="true"></i>
</a> </div>
          </div>

</div>
      
    </div>
  </div>
  </section>
  
<section class="affiliation">
  <div class="container">
  
    <div class="row">
	<h2 class="heading text-center">Our Affiliation</h2>
    	        <div class="col-xs-6 col-sm-4 col-md-3">            
            <div class="thumbnail1">
                <div class="caption1">
                    <h4>Department of Labour</h4>
                    <p>The&nbsp;Department of Labour&nbsp;will play a significant role in reducing unemployment, poverty and inequality through a set of policies and program...</p>
                    <p><a href="http://www.jybce.org/school/20" class="label label-danger">Read More</a></p>
                </div>
                <img src="files/img855.jpg" alt="">
<h5>Department of Labour</h5>
            </div>
        </div>
                <div class="col-xs-6 col-sm-4 col-md-3">            
            <div class="thumbnail1">
                <div class="caption1">
                    <h4>NARC OF INDIA</h4>
                    <p>...</p>
                    <p><a href="http://www.jybce.org/school/21" class="label label-danger">Read More</a></p>
                </div>
                <img src="files/certificate1.jpg" alt="">
<h5>NARC OF INDIA</h5>
            </div>
        </div>
                <div class="col-xs-6 col-sm-4 col-md-3">            
            <div class="thumbnail1">
                <div class="caption1">
                    <h4>NIELTI</h4>
                    <p>...</p>
                    <p><a href="http://www.jybce.org/school/22" class="label label-danger">Read More</a></p>
                </div>
                <img src="files/certificate1.jpg" alt="">
<h5>NIELTI</h5>
            </div>
        </div>
                <div class="col-xs-6 col-sm-4 col-md-3">            
            <div class="thumbnail1">
                <div class="caption1">
                    <h4>MHRD</h4>
                    <p>The essence of Human Resource Development is education, which plays a significant and remedial role in balancing the socio-economic fabric of the Coun...</p>
                    <p><a href="http://www.jybce.org/school/19" class="label label-danger">Read More</a></p>
                </div>
                <img src="files/certificate1.jpg" alt="">
<h5>MHRD</h5>
            </div>
        </div>
            </div>
  
</div>
</section>
  
<!-- <section class="recent">
  <div class="container">
    <div class="row">
      <div class="col-lg-12">
        <h2 class="heading text-center">Recent Works</h2>
        <div class="solidline"></div>
          <div class="clients-carousel custom-carousel touch-carousel navigation-1 owl-carousel owl-theme" data-appeared-items="6" data-navigation="true" style="opacity: 1; display: block;"> 
            
                        <div class="owl-wrapper-outer"><div class="owl-wrapper" style="width: 752px; left: 0px; display: block; transition: all 1000ms ease 0s; transform: translate3d(0px, 0px, 0px);"><div class="owl-item" style="width: 188px;"><div class="client-item item"> <a href="http://www.jybce.org/homepage#"><img src="files/1526242021portfolio-01.jpg" alt=""></a> </div></div><div class="owl-item" style="width: 188px;"><div class="client-item item"> <a href="http://www.jybce.org/homepage#"><img src="files/1526312303portfolio-03.jpg" alt=""></a> </div></div></div></div>
                        
                      <div class="owl-controls clickable" style="display: none;"><div class="owl-buttons"><div class="owl-prev"><i class="fa fa-angle-left"></i></div><div class="owl-next"><i class="fa fa-angle-right"></i></div></div></div></div>
      </div>
    </div>
  </div>
</section> -->
</section>

<?php include "footer.php"; ?>	
<?php include "footer-link.php"; ?>