...
function  getCandidates()
{
	...
	$sql="SELECT pursuing_course.*,student_info.*
		...
		WHERE pursuing_course.current_status='PURSUING' AND pursuing_course.franchise_id='{$_SESSION['franchise_id']}'
		GROUP BY pursuing_course.student_id";
	$res=mysqli_query($conn,  $sql)        ;
	while($row=mysqli_fetch_assoc($res))
	{
		$option.='<option value="'.$row['slno'].'">'.$row['St_Name']."-".$row['Fathers_Name'].'</option>';
	}
	echo $option;
}
function getCourses()
