...
if($_POST){
   ...
   $sql    = "SELECT `marks`.*, `pursuing_course`.`regno`,`student_info`.`St_Name`
            ...
            WHERE `marks`.`printed_at` IS NOT NULL AND `marks`.`franchise_id`='{$franchise}' AND DATE(`marks`.`printed_at`) BETWEEN '{$fromDate}' AND '{$toDate}'" ;
   ...
   if(mysqli_num_rows($ress) > 0){
       ...
       while ($row = mysqli_fetch_assoc($ress)) {
           ...
           $row['printed_date'] = date('d/m/Y', strtotime($row['printed_at']));
           $result['records'][] = $row;
       }
   }else{
